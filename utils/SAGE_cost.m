function cost = SAGE_cost(params,S,TEs,te2)

% ZZjing: dsage fitting (can be multi-groups imgs, every group contains one ge, one mix, one se)
% the size of Sfitting depend on the number of groups
...
elseif size(Sfit,3) == 6
    Sfitting = cat(2,Sfit(:,:,1),Sfit(:,:,2),Sfit(:,:,3),Sfit(:,:,4),Sfit(:,:,5),Sfit(:,:,6));
end
